//! Bidirectional copy with customizable buffer sizes, ported from shoes
//! (src/copy_bidirectional.rs), which itself was forked from tokio.
...
/// Copies data in both directions between `a` and `b` using buffers of the
/// specified size. If an EOF is observed on one stream, shutdown is invoked
/// on the other, and copying in the other direction continues.
